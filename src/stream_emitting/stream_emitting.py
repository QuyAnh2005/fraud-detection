...
def main(args):
    for _ in range(20):
        try:
            ...
            producer = KafkaProducer(
                ...
                client_id="transaction_stats_producer",
            )

            admin = KafkaAdminClient(
                bootstrap_servers=[args.bootstrap_servers],
                client_id="transaction_stats_admin",
            )
    ...
    df = pd.read_parquet(args.data).sort_values(by="step")
    records = df[['step', 'type', 'amount', 'nameOrig', 'oldbalanceOrg', 'newbalanceOrig',
                  'nameDest', 'oldbalanceDest', 'newbalanceDest', 'isFraud',
                  'isFlaggedFraud', 'created']].to_dict("records")

    # simulate streaming events by increase one hour for every records
    iteration = 1
    while True:
        for record in records:
            record["step"] = (
                record["step"] + pd.Timedelta(hours=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--topic_name', default="transactions",
                        help='Kafka topic name')
    parser.add_argument('-d', '--data', default="data/sample_dataset_stream.parquet",
